 ...
 * @param {string|number} room_id - The id of the room which has been created
 * @constructor
 * @return {Object}
 */
...
module.exports = function waiting_rooms(min_room_size, max_room_size) {
	/**
	A two-level array containing a game_id pointing to an arrays containing the rooms which have been created for that game
	@access private
	*/
	this.games = [];
	...
	@param {socket} user_socket - The socket to be added to a game
	@param {number} game_id - The id of the game to which the user should be added
	@return {JSON} 
	@access public
	*/
	this.add_to_room = function(user_socket, game_id) {
		...
		this.number_in_room++;
		return {'room':open_room,'user_number':this.number_in_room,'full':open_room.ready};
		
	}
	
	/**
	 * Find rooms with the given game id which are open and creates rooms when none are available
	 * @param {socket} socket - The socket to which the client is attached
	 * @param {number|string} game_id - The id of the game which requires a room
	...
	*/
	this.find_open_rooms = function(socket, game_id){
		...
		if(this.games.length === 0 
			...
			available_rooms = this.games[game_id].filter(function(element, index,array){
	...
	*/
	 ...
	 * @param {socket} socket - The socket to which the client is attached
	 * @param {string} socket_id - The id of the socket to which the client is attached
	*/
