	...
	var game_finished = function(board){//Check if a win condition has been met
		...
		for(var i=0;i<row_length;i++){
			for(var j=0;j<row_length-1;j++){
				if(board[(i*row_length) + j ]==(board[(i*row_length)+j + 1]) && board[(i*row_length) + j]!=""){
					number_in_row++;
				}
			}
		...
		for(var i=0;i<row_length;i++){
			for(var j=0;j<row_length-1;j++){
				if(board[(row_length*j)+i]==(board[((row_length*j)+i) + row_length]) && board[(row_length*j)+i]!=""){
					number_in_row++;
				}
			}
		...
		//Check Angles
			...
			for(var j=0;j<row_length-1;j++){
				if(board[(j*row_length) + j]==(board[((j+1)*row_length) + j + 1]) && board[(j*row_length) + j]!=""){
					number_in_row++;
				}
				if(board[(row_length -1) + ((row_length -1)*j) ] ==board[(row_length -1) + ((row_length -1)*(j+1)) ] && board[(row_length -1) + ((row_length -1)*j) ]!=""){
					number_in_second_angle++;
				}
			}
			
			if(number_in_row == row_length-1 || number_in_second_angle==row_length-1){
				return true;//Game has finished
			}{
				number_in_row=0;
			}
